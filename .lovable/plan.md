...
### 5. Doppelbewerbungs-Schutz noch nicht aktiv
Die Migration `20260819000000_applications_dedupe_60d.sql` liegt bereit, ist auf dem Backend aber noch nicht eingespielt — Doppelbewerbungen und Doppelmails sind weiter möglich.

### 6. Öffentlicher E-Mail-Lookup ist ungebremst
`/api/public/application-lookup` verrät ohne Begrenzung, ob zu einer E-Mail eine Bewerbung existiert, und leitet direkt ins Gespräch. Wer eine fremde Adresse kennt, kommt ins fremde Interview.
...
**Stufe 2 — Absicherung**
- Doppelbewerbungs-Migration einspielen (60-Tage-Fenster, spätere Wiederbewerbung bleibt möglich).
- Rate-Limit und neutrale Antworten für den öffentlichen E-Mail-Lookup; bei Treffer weiterhin direkte Weiterleitung, aber pro IP gedrosselt.

**Stufe 3 — Conversion-Feinschliff**
...
- Migration analog `20260818000000_reminder_kinds_full.sql` um `interview_abandoned` erweitern; `20260819000000_applications_dedupe_60d.sql` deployen.
- `src/routes/api/public/application-lookup.ts`: IP-basiertes Rate-Limit über die vorhandenen Bot-Shield-Utilities.